...
{
    private static readonly Lazy<IReadOnlyList<AuthBootstrapPacketDefinition>> CapturedAnkalinePackets =
        new(BuildCapturedAnkalinePackets);
    ...
    private static IReadOnlyList<AuthBootstrapPacketDefinition> BuildCapturedAnkalinePackets()
    {
        ...
        while (offset < rawBlob.Length)
        {
            ...
            packets.Add(
                new AuthBootstrapPacketDefinition(
                    ...
                    DescribeCapturedPacket(decodedPacket.MessageId, decodedPacket.Payload)));
    ...
    private static string DescribeCapturedPacket(ushort messageId, byte[] payload)
    {
        ...
        {
            case 6253:
                builder.Append(DescribeRawDataPayload(payload));
                break;
    ...
    private static string DescribeRawDataPayload(byte[] payload)
    {
        var builder = new StringBuilder();
